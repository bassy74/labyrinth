...
def walk(goal,Q,turn):
    while turn > 0:
        ...
        while(s != goal[0] and s != goal[1]):
            ...
            if check(s) == False:
                Q[s_pre[0]+6*s_pre[1]-7][a] = (1 - alpha) * Q[s_pre[0]+6*s_pre[1]-7][a] + alpha * (-0.1)
                s = copy.copy(s_pre)
                           
            
            elif check(s) == True and s == goal[0]:
                Q[s_pre[0]+6*s_pre[1]-7][a] = (1 - alpha) * Q[s_pre[0]+6*s_pre[1]-7][a] + alpha * (2 + ganma * max(Q[s[0]+6*s[1]-7]))
                               
            elif check(s) == True and s == goal[1]:
                Q[s_pre[0]+6*s_pre[1]-7][a] = (1 - alpha) * Q[s_pre[0]+6*s_pre[1]-7][a] + alpha * (1 + ganma * max(Q[s[0]+6*s[1]-7]))
            else:
                Q[s_pre[0]+6*s_pre[1]-7][a] = (1 - alpha) * Q[s_pre[0]+6*s_pre[1]-7][a] + alpha * ganma * max(Q[s[0]+6*s[1]-7])
